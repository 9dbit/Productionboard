...
import { storyboardTileForShot } from '@/lib/production-assets';
...
export default async function EpisodeStoryboardPage({params}:{params:Promise<{episodeId:string}>}){
  ...
  return <div className="movieWorkspace page">
    ...
    <section className="episodeStoryboardHeader"><div><span className="eyebrow accent">STORYBOARD • {ep.id}</span><h2>{ep.title}</h2><p>{ep.duration}s • {ep.scenes.length} scenes • {ep.scenes.reduce((n,s)=>n+s.shots.length,0)} shots</p></div><div className="episodeHeaderActions"><StatusBadge status={ep.status}/><Link href="/movies/casablanca/script" className="button button-secondary">Open mapped script</Link></div></section>
    {ep.scenes.length===0?<div className="emptyState">Storyboard belum dibuat untuk episode ini.</div>:<>
      ...
      <div className="storyboardSceneStack">{ep.scenes.map(scene=><section className="storyboardScene" key={scene.id}><header><div><span className="eyebrow">{scene.id}</span><h3>{scene.title}</h3><p>{scene.objective}</p></div><span className={`horror horror-${scene.horror.toLowerCase()}`}>{scene.horror}</span></header><div className="sceneProductionMeta"><span>{scene.timecode}</span><span>Story {scene.storyClock}</span><span>{scene.locationId}</span><span>{scene.environmentId}</span><span>{scene.characters.join(', ')}</span></div><div className="storyboardShotGrid">{scene.shots.map(shot=>{const tile=storyboardTileForShot(shot.id);return <article className="storyboardShot" key={shot.id}>{tile?<AtlasVisual tile={tile} alt={shot.id}/>:<AssetVisual src={shot.thumbnailUrl} alt={shot.id} downloadable/>}<div className="storyboardShotBody"><div className="shotTitle"><div><span className="eyebrow">{shot.id}</span><h4>{shot.camera}</h4></div><StatusBadge status={shot.status}/></div><div className="shotTimeRow"><strong>{shot.timecode}</strong><span>{shot.storyClock} WIB</span><span>{shot.horror}</span></div><p>{shot.action}</p><dl><div><dt>Site</dt><dd>{shot.locationId}</dd></div><div><dt>Environment</dt><dd>{shot.environmentId}</dd></div><div><dt>Audio</dt><dd>{shot.audio}</dd></div><div><dt>Characters</dt><dd>{shot.characterIds.join(', ')}</dd></div></dl><details className="shotPrompt"><summary>AI prompt & continuity</summary><p>{shot.prompt}</p><small>{shot.continuity}</small></details><div className="cardActions"><Link href={`/shots/${shot.id}`} className="button button-secondary">Open shot inspector</Link></div></div></article>})}</div></section>)}</div>
    </>}
  </div>;
}
