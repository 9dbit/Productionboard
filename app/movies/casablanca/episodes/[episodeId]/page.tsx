...
import { storyboardImageForShot } from '@/lib/visual-overrides';
...
export default async function EpisodeStoryboardPage({params}:{params:Promise<{episodeId:string}>}){
  ...
  return <div className="movieWorkspace page">
    ...
    {ep.scenes.length===0?<div className="emptyState">Storyboard belum dibuat untuk episode ini.</div>:<>
      ...
      <div className="storyboardSceneStack">{ep.scenes.map(scene=><section className="storyboardScene" key={scene.id}><header><div><span className="eyebrow">{scene.id}</span><h3>{scene.title}</h3><p>{scene.objective}</p></div><span className={`horror horror-${scene.horror.toLowerCase()}`}>{scene.horror}</span></header><div className="sceneProductionMeta"><span>{scene.timecode}</span><span>Story {scene.storyClock}</span><span>{scene.locationId}</span><span>{scene.environmentId}</span><span>{scene.characters.join(', ')}</span><span>FRAME 9:16</span></div><div className="storyboardShotGrid">{scene.shots.map(shot=>{const src=storyboardImageForShot(shot.id)||shot.thumbnailUrl;return <article className="storyboardShot" key={shot.id}><AssetVisual src={src} alt={`${shot.id} vertical storyboard`} downloadable className="storyboardVerticalMedia"/><div className="storyboardShotBody"><div className="shotTitle"><div><span className="eyebrow">{shot.id}</span><h4>{shot.camera}</h4></div><StatusBadge status={shot.status}/></div><div className="shotTimeRow"><strong>{shot.timecode}</strong><span>{shot.storyClock} WIB</span><span>{shot.horror}</span><span>9:16 NATIVE</span></div><p>{shot.action}</p><dl><div><dt>Site</dt><dd>{shot.locationId}</dd></div><div><dt>Environment</dt><dd>{shot.environmentId}</dd></div><div><dt>Audio</dt><dd>{shot.audio}</dd></div><div><dt>Characters</dt><dd>{shot.characterIds.join(', ')}</dd></div></dl><details className="shotPrompt"><summary>AI prompt & continuity</summary><p>{shot.prompt}</p><small>{shot.continuity}</small></details><div className="cardActions"><Link href={`/shots/${shot.id}`} className="button button-secondary">Open shot inspector</Link></div></div></article>})}</div></section>)}</div>
    </>}
  </div>;
}
