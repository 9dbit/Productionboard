...
import { actualCharacterAngles, characterAngleImage } from '@/lib/visual-overrides';
...
export default function MovieAssetsPage(){
  ...
  return <div className="movieWorkspace page">
    ...
    <section className="workspaceIntro"><div><span className="eyebrow accent">ASSET LIBRARY</span><h2>Character locks, sites & environments</h2></div><p>Enam angle character lock sekarang memakai file image individual 600×800. Browser tidak lagi melakukan crop dari contact sheet.</p></section>

    <section className="assetSection"><div className="sectionHead"><div><span className="eyebrow">CHARACTER ASSETS</span><h2>Character lock library</h2></div><span className="muted">6 standalone real images per locked character</span></div>
      <div className="characterAssetStack">{characters.map(character=>{const preview=characterAngleImage(character.id,'FRONT');return <article className="characterLockCard" key={character.id}>
        <div className="characterLockIntro">{preview&&<AssetVisual src={preview} alt={`${character.name} front`} className="characterHeroVisual"/>}<div><div className="assetCardHead"><span className="eyebrow">{character.id}</span><StatusBadge status={character.status}/></div><h3>{character.name}</h3><p>{character.description}</p><div className="assetUsageSummary"><span>{character.episodeIds?.join(', ')||'Future arc'}</span><span>6 standalone angles loaded</span></div><p className="muted">{character.storyUse}</p></div></div>
        <div className="angleGrid">{actualCharacterAngles.map(angle=>{const src=characterAngleImage(character.id,angle);return <div className="angleSlot lockedAngle" key={angle}>{src&&<AssetVisual src={src} alt={`${character.name} ${angle}`} className="characterAngleVisual"/>}<span>{angle}</span><small>Individual locked face reference</small></div>})}</div>
        <div className="missingAngleStrip"><span className="eyebrow">REMAINING ANGLES TO GENERATE</span><div className="tagRow">{missingAngles.map(angle=><span className="metaChip" key={angle}>{angle}</span>)}</div><p className="muted">Tidak ditampilkan sebagai gambar palsu. Enam angle tambahan nantinya harus digenerate dari face lock yang sama untuk mencapai full 12-angle character lock.</p></div>
      </article>})}</div>
    </section>
